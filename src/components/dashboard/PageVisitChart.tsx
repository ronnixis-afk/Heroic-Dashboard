...
export function PageVisitChart() {
  ...
  const getBarColor = (percentage: number) => {
    if (percentage >= 25) return '#20cce0';
    if (percentage >= 10) return '#3ecf8e';
    if (percentage >= 5) return '#a855f7';
    return '#8b8c94';
  };

  return (
    <div className="card p-3.5 h-80 flex flex-col">
      <h3 className="card-title mb-3">Page Visits (30 Days)</h3>
      
      <div className="flex-1 w-full min-h-0 relative">
        {loading && (
          <div className="absolute inset-0 z-10 flex flex-col gap-2 px-2 py-4 opacity-20 pointer-events-none">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="shimmer h-3 rounded-md" style={{ width: `${Math.random() * 60 + 20}%` }} />
            ))}
          </div>
        )}
            ...
            <Tooltip 
              ...
              content={({ active, payload }) => {
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="tooltip-panel pointer-events-none w-[220px]">
                        <p className="text-xs font-semibold text-white m-0 mb-2">{label}</p>
                        <div className="space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="text-xs text-brand-text-muted">Total Visits</span>
                            <span className="text-xs font-semibold text-white">{entry.visits.toLocaleString()}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-xs text-brand-text-muted">Usage Share</span>
                            <span className="text-xs font-semibold text-brand-accent">{entry.percentage}%</span>
                          </div>
                          <div className="flex justify-between items-center">
