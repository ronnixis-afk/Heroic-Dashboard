    ...
    <div className="glass-panel p-6 h-[340px] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-1 pr-1">
        {(isLoading ? Array.from({ length: 5 }) : topConsumers).map((tx, idx) => (
          <div key={isLoading ? idx : tx.id} className={`flex items-center p-2 rounded-xl transition-colors ${idx === 1 && !isLoading ? 'bg-[#292a32]' : 'hover:bg-[#292a32]'}`}>
            ...
            {isLoading ? (
              <SkeletonText width={60} className="h-3 w-24 mx-auto" />
            ) : (
              <span className="text-[11px] text-[#8b8c94] w-24 text-center shrink-0">{tx.model}</span>
            )}
            {isLoading ? (
              <SkeletonText width={50} className="h-3 w-20 ml-auto" />
            ) : (
