...
- **Upload dropdowns:** Item Image → Item Category (Weapons, Protection, …) then **Art Family** (family name only, with rolled-up image counts). Selecting a Weapons/Protection family shows “Also Covers: …” for folded chassis. Match keys: `metadata.itemCategory` + `metadata.itemSubtype` (family) → RPG `Item.imageUrl` via `templateName` folded through `resolveItemArtFamily`.
