...
| `/admin/roadmap` | Product Roadmap | CMS for marketing roadmap milestones (`RoadmapItem`). Hook: `src/hooks/useRoadmap.ts` → Supabase JWT CRUD (same pattern as News). Fields: title, summary, details, phase (Near Horizon / Next Wave / On The Horizon / Shipped), status (In Dev / Queued / Planned / Shipped), category, featured, published, sortOrder. RPG reads published rows via public `GET /api/roadmap-items` for `/roadmap` and landing `RoadmapSection`. |
