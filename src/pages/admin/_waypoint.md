...
| `/admin/media` | Media Library | Storage upload via Supabase (`getAdminSupabase` + RLS); `ImageAsset` metadata CRUD/list via RPG `/api/admin/image-assets*`. Dynamic realm race discovery via `/api/admin/image-assets/discovered-races` with "Needs Artwork" zero-portrait filter. Zone Image uploads use **Terrain Type** (`metadata.terrainType`), not narrative zone properties. |
