...
- **Weight categories:** Sync also writes `ITEM_WEIGHT_CATEGORY_BY_GENRE` (Light / Medium / Heavy / Shield) from weapon loot tags and `armorStats.armorType`. Art Family option labels for Weapons/Protection show e.g. `Light - Laser Pistol` (display only — not uploaded as a tag).
