...
| `/admin/news` | Global News & Patch Notes | Dynamic App Version manager (`app_version`), Announcements CMS, Public Patch Notes publisher, and real-time App Update Popup overlay manager with feature highlight builder and 1-click Push/Activate. Overview/Content fields use `RichTextEditor` (bold, italic, bullet/numbered lists) stored as allowlisted HTML; rendered via `RichTextContent` / `sanitizeRichHtml` in dashboard preview and RPG patch notes + in-game popup. |
| `/admin/roadmap` | Product Roadmap | CMS for marketing roadmap milestones (`RoadmapItem`). Hook: `src/hooks/useRoadmap.ts` → Supabase JWT CRUD (same pattern as News). Fields: title, summary, phase (Near Horizon / Next Wave / On The Horizon / Shipped), status (In Dev / Queued / Planned / Shipped), category, featured, published, sortOrder. Legacy `details` column is cleared on write and not shown in admin or on `/roadmap`. RPG reads published rows via public `GET /api/roadmap-items` and renders them verbatim on `/roadmap` (`RoadmapClientPage`) — no client-side milestone copy. |
| `/admin/media` | Media Library | WebP asset upload/tagging; `useImageAssets` loads one server page at a time (`IMAGE_ASSET_PAGE_SIZE` = 60) with server-side search, genre, asset type, and tag filters. Genre filter omits "Any Genre" (treats it as All). Upload + library dropdown options show faceted counts as `Label (N)` from a lightweight `genre/assetType/metadata/tags` facet query (`imageAssetFacetCounts.ts`) that **pages through the full `ImageAsset` table** (PostgREST max_rows ≈ 1000; a single unpaged select under-counted later asset types like Monster Portrait); child counts respect selected genre / asset type / cascading metadata parents. **Monster Portrait** is available under Fantasy / Sci-Fi / Modern / Any Genre; cascading Monster Type / Monster Subtype metadata auto-tags for filtering (`monsterPortraitCatalog.ts`). Upload Humanoid role subtypes (Guard, Soldier, Thug, Bandit, Pirate, Street Fighter) per genre so Fantasy guards never match Sci-Fi art. RPG match prefers world genre, then Any Genre fallback. **NPC Portrait** is uploadable (race/gender metadata) and used when System Settings NPC Image Source is Database (default); Nano Banana 2 Lite remains an optional AI source with Database fallback. **Character Portrait** / **Portrait Race** dropdown = core five + 10 shared suggested customs (`SUGGESTED_PORTRAIT_RACES`: Vampire, Werewolf, Goblin, Troll, Demon, Angel, Undead, Ghost, Giant, Fey) + distinct `metadata.race` from uploaded Character portraits on the current page (same genre or Any Genre) + local customs typed this browser. **Mount Portrait** / **Vehicle Portrait** / **Ship Portrait** show a **Template Name** dropdown from the synced rideable catalog (`rideablePortraitCatalog.ts`: 8 mounts + 8 vehicles + 8 ships per genre); library tiles and the RPG gallery caption that template name. Match keys: `metadata.mountType` / `vehicleType` / `shipType` → companion `imageUrl`. **Item Image** uses cascading **Item Category** + **Art Family** from `itemPortraitCatalog.ts` (family-folded Weapons/Protection names such as Longbow / Heavy Crossbow / Padded Armor); dropdown counts roll retired chassis into the family; tiles caption the art family; RPG match folds `templateName` via `resolveItemArtFamily` → `/api/image-assets/match`. |
...
- **Source of truth:** RPG loot blueprints — `weaponLootTemplates.ts` and `armorLootTemplates.ts` (per-genre Fantasy / Modern / Sci-Fi tables), `itemRegistry.ts` (`LOOT_TABLES`), `itemArtFamilies.ts` (`ITEM_ART_FAMILIES_BY_GENRE`), and `materials.ts` typeTags. Runtime helper: `heroic-ai-rpg/src/constants/itemPortraitCatalog.ts`.
...
- **Upload dropdowns:** Item Image → Genre → Item Category (Weapons, Protection, …) then **Art Family** (family name only for that genre, with rolled-up image counts). Selecting a Weapons/Protection family shows “Also Covers: …” for folded chassis. Match keys: `genre` + `metadata.itemCategory` + `metadata.itemSubtype` (family) → RPG `Item.imageUrl` via `templateName` folded through `resolveItemArtFamily(name, genre)`.

## Insights Data Sources

Analytics / PII views are **not** readable via PostgREST (anon grants revoked). All metric reads go through Clerk-gated RPG admin APIs (`fetchRpgAdmin` → `/api/admin/analytics/*`) using the **standard Clerk session token**. Use `getToken({ template: 'supabase' })` only for remaining PostgREST tables with RLS (`User`, `News`, `RoadmapItem`, `ImageAsset`, `Feedback`, `SurveyResponse`, `CreditAdjustment`).
