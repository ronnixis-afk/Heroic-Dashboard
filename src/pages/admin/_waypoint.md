...
| `/admin/media` | Media Library | WebP asset upload/tagging; `useImageAssets` loads one server page at a time (`IMAGE_ASSET_PAGE_SIZE` = 60) with server-side search, genre, asset type, and tag filters. Genre filter omits "Any Genre" (treats it as All). **Monster Portrait** is available under Fantasy / Sci-Fi / Modern / Any Genre; cascading Monster Type / Monster Subtype metadata auto-tags for filtering (`monsterPortraitCatalog.ts`). Upload Humanoid role subtypes (Guard, Soldier, Thug, Bandit, Pirate, Street Fighter) per genre so Fantasy guards never match Sci-Fi art. RPG match prefers world genre, then Any Genre fallback. **NPC Portrait** is uploadable (race/gender metadata) and used when System Settings NPC Image Source is Database (default); Nano Banana 2 Lite remains an optional AI source with Database fallback. **Character Portrait** / **Portrait Race** dropdown = core five + 10 shared suggested customs (`SUGGESTED_PORTRAIT_RACES`: Vampire, Werewolf, Goblin, Troll, Demon, Angel, Undead, Ghost, Giant, Fey) + distinct `metadata.race` from uploaded Character portraits on the current page (same genre or Any Genre) + local customs typed this browser. **Mount Portrait** / **Ship Portrait** show a **Template Name** dropdown (stables/shipyard names like War Destrier, Elven Stag) after the asset type is selected; library tiles and the RPG gallery caption that template name. Create-time match uses `metadata.mountType` / `shipType` → companion `imageUrl`. |
