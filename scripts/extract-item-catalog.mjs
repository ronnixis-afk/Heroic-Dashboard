 ...
 *
 * Weapons and Protection subtypes are folded through ITEM_ART_FAMILIES so the
 * upload dropdown lists art-family names (not every mechanical chassis).
 *
...
/**
 * Parse ITEM_ART_FAMILIES quoted key/value pairs.
 * Only string-literal entries inside the object body are read (comments are skipped
 * because they do not match the quote pair pattern in active code).
 */
const extractArtFamilies = (familiesSrc) => {
  const map = new Map();
  if (!familiesSrc) return map;

  const keyRe = /ITEM_ART_FAMILIES(?:\s*:\s*Record<[^>]+>)?\s*=\s*\{/;
  const keyMatch = keyRe.exec(familiesSrc);
  ...
  const body = familiesSrc.slice(start, end + 1);
  // Strip line comments so optional-tier pairs stay inactive when commented out.
  const withoutLineComments = body.replace(/\/\/.*$/gm, '');
  const pairRe = /(?:'((?:\\'|[^'])*)'|"((?:\\"|[^"])*)")\s*:\s*(?:'((?:\\'|[^'])*)'|"((?:\\"|[^"])*)")/g;
  let match;
  while ((match = pairRe.exec(withoutLineComments)) !== null) {
    const member = (match[1] || match[2] || '').replace(/\\'/g, "'").replace(/\\"/g, '"').trim();
    const family = (match[3] || match[4] || '').replace(/\\'/g, "'").replace(/\\"/g, '"').trim();
    if (member && family) map.set(member, family);
  }
  return map;
};
...
const artFamilies = extractArtFamilies(familiesSrc);

const weapons = foldThroughFamilies(
  extractQuotedNames(weaponsSrc, /w\(\s*(?:'((?:\\'|[^'])*)'|"((?:\\"|[^"])*)")/),
  artFamilies
);
const armors = foldThroughFamilies(extractTableNames(registrySrc, 'armors'), artFamilies);
...
export const ITEM_ART_FAMILIES: Readonly<Record<string, string>> = ${serializeArtFamilies(artFamilies)};
...
console.log(
  `[sync:item-catalog] Wrote ${catalog.length} categories / ${totalSubtypes} subtypes ` +
    `(${artFamilies.size} art-family mappings) to ${path.relative(dashboardRoot, outPath)}`
);
